...
表情改変をすると、目のBlendShapeが破綻し、ゲーム内で目が潰れたり、閉じきらなくなったりすることがあります。
Avatar Blink Fixは、その原因となる目のBlendShapeを修正するツールです。
...
修正は、ヒエラルキーでアバターを右クリックして、［Avatar Blink Fix］→［自動修正（推奨）］を選ぶだけです。
100体以上のアバターに対応するAuto Fixプリセットがあるので、すぐに修正できます。
プリセットがないアバターも、修正したい項目にチェックを入れれば修正できます。

まばたきだけでなく、ウィンク、MMD用BlendShape、フェイストラッキング用BlendShapeも修正できます（コンプリート版）。
...
無料版では、普段のまばたきに使うBlendShapeを修正できます。修正したアバターは、そのままゲーム内で使えます。
...
特別な理由がなければ、VCC / ALCOMから導入してください。最新のAvatar Blink FixとAuto Fixプリセットを、まとめて簡単に導入できます。
