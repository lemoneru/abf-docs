...
::: warning 自動修正の対象はBlendShapeです
**既存の表情アニメーションに設定された数値や動きを、自動で編集・修正する機能ではありません。**<br>
修正するのは、目を閉じるBlendShapeです。

アニメーションとの競合は、[自動修正（ベイクモード）](/guide/bake)で軽減できる可能性があります。すべての破綻が解消されるわけではなく、アニメーション側の調整が必要な場合もあります。

まばたきと表情アニメーションの合成（ベータ機能）は、自動修正とは別の機能です。
