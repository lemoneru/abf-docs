...
ツールを入れただけの状態が、そのまま**無料のトライアル版**です。
直せるのは自動選択された1つのBlendShapeだけですが、**操作は有料版とまったく同じ**。
**自分のアバターがちゃんと直るか、操作感はどうか**を確かめてから決められます。

**まばたきの破綻を直したいだけなら、無料版のままでも大丈夫です。** 修正されたBlendShapeはそのまま使い続けられます。
ほかのBlendShapeも直したくなったら、**有料版へは簡単にアップグレードできます**（同梱のファイルを1回インポートするだけ。同じPCなら他のプロジェクトでもそのまま使えます）。
